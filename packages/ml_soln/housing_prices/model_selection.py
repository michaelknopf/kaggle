...
class HousingPricesModelSelection:
    ...
    def _grid_search(self, grid, cv_splits=5):
        grid = ctx().model_selection.pre_process_grid(grid, prefix=HYPERPARAM_PREFIX)
        cv = ctx().model_selection.simple_grid_search_cv(estimator=ctx().model.pipeline,
                                                         param_grid=grid,
                                                         cv_splits=cv_splits,
                                                         scoring=SCORE_FUNCTION,
                                                         use_halving=True)
        ctx().model_selection.grid_search(cv, self.X, self.y)
...
if __name__ == '__main__':
    ctx().model_selection.housing_grid_search_2()
