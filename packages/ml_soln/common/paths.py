...
def paths_for_package_name(package_name: str,
    ...
    if is_sagemaker:
        return sagemaker_paths(package_name)
    else:
        return local_paths(package_name, job_name)

@cache
def sagemaker_paths(package_name: str):
    """
    Example folder structure when running in docker container:
        ...
        └── ml/
            ...
            └── code/
                └── entrypoint.py
    """
    package_dir = make_package_dir(package_name)
...
@cache
def local_paths(package_name: str,
                job_name: str = sm_utils.job_name):
    """
